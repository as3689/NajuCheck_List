...
//     body: JSON.stringify({ name, columns: ['점검항목'] })
//   });
...
// function showColumnModal(checklistId, columns) {
//   console.log('Opening column modal for checklistId:', checklistId);
//   const modal = document.createElement('div');
//   modal.className = 'modal';
//   modal.innerHTML = `
//     <div class="modal-content">
//       <h3>새 열 추가</h3>
//       <input type="text" id="columnNameInput" placeholder="열 이름을 입력하세요 (예: 5/30)" />
...
//     const newColumn = input.value.trim();
//     if (newColumn) {
//       const updatedColumns = [...columns, newColumn];
//       console.log(`Updating columns for checklist ${checklistId}:`, updatedColumns);
//       const updateResponse = await fetch(`/api/checklists/${checklistId}/columns`, {
//         method: 'PUT',
//         headers: { 'Content-Type': 'application/json' },
...
//         const itemsResponse = await fetch(`/api/items/${checklistId}`);
...
//       } else {
//         console.error('Failed to update columns:', updateResult.message);
//         alert('열 추가에 실패했습니다.');
//       }
//     }
...
//     <div class="modal-content">
//       <h3>새 항목 추가</h3>
//       <input type="text" id="itemNameInput" placeholder="항목 이름을 입력하세요 (예: A상권)" />
//       <div class="modal-buttons">
//         <button id="modalConfirmBtn">추가</button>
//         <button id="modalCancelBtn">취소</button>
//       </div>
...
//     if (!response.ok) {
//       console.error('Failed to load snapshot:', response.status, response.statusText);
//       alert('스냅샷 데이터를 로드하지 못했습니다.');
//       return;
//     }
//     const snapshot = await response.json();
//     console.log('Snapshot data:', snapshot);
//     if (!snapshot || !snapshot.columns || !snapshot.items) {
...
//     addColumnButton.addEventListener('click', () => addColumn(currentTableId, snapshot.columns));
//     thAdd.appendChild(addColumnButton);
...
//     addButtonCell.colSpan = snapshot.columns.length + 1;
//     addButtonCell.style.textAlign = 'center';
//     const addButton = document.createElement('button');
//     addButton.textContent = '항목 추가';
//     addButton.onclick = () => addItem(currentTableId, snapshot.columns);
...
//         e.preventDefault();
//         const updatedRows = Array.from(tbody.querySelectorAll('tr')).filter(row => row !== addButtonRow);
...
//     });
//   } catch (error) {
//     console.error('Error in loadTableByDate:', error.message);
...
//     console.log('Saving snapshot:', { tableId: currentTableId, snapshotDate, columns: checklist.columns, items, username });
...
//       alert('체크리스트 저장에 실패했습니다: ' + result.message);
//     }
//   } catch (error) {
//     console.error('Error in saveChecklist:', error.message);
//     alert('체크리스트 저장 중 오류가 발생했습니다.');
//   }
// }

// async function addColumn(checklistId, columns) {
//   console.log('addColumn called with checklistId:', checklistId, 'columns:', columns);
//   showColumnModal(checklistId, columns);
// }

// async function addItem(checklistId, columns) {
//   console.log('addItem called with checklistId:', checklistId, 'columns:', columns);
//   showItemModal(checklistId, columns);
// }
...
//   } catch (e) {
//     console.error('Failed to parse checklists response:', e.message);
//     return;
//   }
//   if (!checklists) {
...
//   const thead = document.createElement('thead');
//   const headerRow = document.createElement('tr');
//   console.log('Raw columns value:', checklist.columns);

//   let columns;
//   try {
//     if (typeof checklist.columns === 'string') {
//       columns = JSON.parse(checklist.columns);
//     } else {
//       columns = checklist.columns;
//     }
//     if (!Array.isArray(columns) || columns.length === 0) {
//       columns = ['점검항목'];
//       await fetch(`/api/checklists/${checklist.id}/columns`, {
//         method: 'PUT',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ columns: ['점검항목'] })
//       });
//     }
//   } catch (e) {
//     console.error('Invalid columns JSON:', e.message);
//     columns = ['점검항목'];
//   }
...
//   const addColumnButton = document.createElement('button');
//   addColumnButton.textContent = '+';
//   addColumnButton.onclick = () => addColumn(checklist.id, columns);
//   thAdd.appendChild(addColumnButton);
//   headerRow.appendChild(thAdd);
//   thead.appendChild(headerRow);
//   tableElement.appendChild(thead);
...
//   } catch (e) {
//     console.error('Failed to parse items response:', e.message);
//     return;
//   }

//   if (items.length === 0) {
//     const initialItems = [
//       { data: { '점검항목': `${checklist.name} A상권` }, display_order: 0 },
//       { data: { '점검항목': `${checklist.name} B상권` }, display_order: 1 }
//     ];
//     for (let i = 0; i < initialItems.length; i++) {
//       const item = initialItems[i];
//       columns.forEach(col => {
//         if (col !== '점검항목') item.data[col] = '';
//       });
//       console.log(`Creating initial item for checklist ${checklist.id}:`, item);
//       const response = await fetch('/api/items', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
...
//   const addButton = document.createElement('button');
//   addButton.textContent = '항목 추가';
//   addButton.onclick = () => addItem(checklist.id, columns);
//   addButtonCell.appendChild(addButton);
//   addButtonRow.appendChild(addButtonCell);
//   tbody.appendChild(addButtonRow);
...
//     alert('로그아웃 후 페이지 이동에 실패했습니다. 새로고침해 주세요.');
//   }
// }
...
function showItemModal(checklistId, columns) {
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <h3>새 항목 추가</h3>
      <input type="text" id="itemNameInput" placeholder="점검내용 입력 (예: A상권 점검)" />
      <div class="modal-buttons">
        <button id="modalConfirmBtn">추가</button>
        <button id="modalCancelBtn">취소</button>
      </div>
...
async function loadTableByDate(date) {
  ...
  try {
    ...
    const addButtonRow = document.createElement('tr');
    const addButtonCell = document.createElement('td');
    addButtonCell.colSpan = snapshot.columns.length;
    addButtonCell.style.textAlign = 'center';
    const addButton = document.createElement('button');
    addButton.textContent = '항목 추가';
...
async function showTable(name) {
  ...
  const thead = document.createElement('thead');
  const headerRow = document.createElement('tr');
  const columns = ['점검내용', '결과', '비고', '작성자', '날짜'];
  columns.forEach(col => {
    const th = document.createElement('th');
    th.textContent = col;
    headerRow.appendChild(th);
  ...
  if (items.length === 0) {
    const initialItems = [
      { data: { '점검내용': `${checklist.name} A상권 점검`, '결과': false, '비고': '', '작성자': '', '날짜': '' }, display_order: 0 },
      { data: { '점검내용': `${checklist.name} B상권 점검`, '결과': false, '비고': '', '작성자': '', '날짜': '' }, display_order: 1 }
    ];
    for (let i = 0; i < initialItems.length; i++) {
      const item = initialItems[i];
  ...
  rows.forEach(row => {
    ...
    row.addEventListener('drop', async (e) => {
      ...
      try {
        ...
        if (!result.success) {
          console.error('Failed to reorder items:', result.message);
          alert('항목 순서 변경에 실패했습니다: ' + (result.message || '알수 없는 오류'));
        } else {
          console.log('Items reordered successfully');
        }
      } catch (error) {
